...
static void *load_file(const char *fn, unsigned *_sz, enum mtk_type type)
{
    ...
    if(read(fd, &data[offset], datasz) != datasz) {
		printf( "[-]read fail %d\n", datasz );
		goto oops;
    }
    close(fd);
...
int main(int argc, char **argv)
{
	...
	int repack_mtk = 0;
	enum mtk_type re_mtk_type = MTK_NONE;
    int fd;
    SHA_CTX ctx;
    const uint8_t* sha;
	...
	re_mtk_type = repack_mtk ? MTK_KERNEL: MTK_NONE;	
	kernel_data = load_file(kernel_fn, &hdr.kernel_size, re_mtk_type);
    if(kernel_data == 0) {
    ...
    if(ramdisk_fn == 0) {
		...
		re_mtk_type = repack_mtk ? MTK_ROOTFS : MTK_NONE;
        ramdisk_data = load_file(ramdisk_fn, &hdr.ramdisk_size,re_mtk_type);
        if(ramdisk_data == 0) {
